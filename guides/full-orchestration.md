...
Root 在整个线程固定使用 GPT-6 Sol High，负责风险分级、架构、协调、集成和验收。
不要中途切换 root 模型或推理强度，以保留 prompt cache 连续性。

优先使用配置好的角色；不为每份子任务报告强制核对 rollout 身份、模型和
effort。`pro_guard.py attest` 保留为怀疑配置漂移时的按需诊断命令，不是阶段
或验收门槛，也不是安全认证。
...
R1-R3 先由 auditor 区分用户观察、原因推测或拟议机制、期望结果，沿真实代码路径
查证，并寻找反例、替代原因及已正常工作的相邻模式。把关键 claim 分类为
`CONFIRMED`、`PARTIALLY_CONFIRMED`、`CONTRADICTED` 或 `UNKNOWN`。
报告必须包含证据、`CLEAR/BLOCK`、预期与不变行为、相关模式/状态、用户可见
反馈、具体验收样例和残余不确定性。若机制不能实现期望行为、改变已正确的相邻
行为，或结果依赖被否定/未解决的关键事实，则标记 `BLOCK`；
